...
{
    const auto table = json::parse(R"([{"a":"1","b":"2"},{"a":"3","b":"4"}])");
...
{
    const auto table = json::parse("[\n  { \"a\" : \"1\" , \"b\":\"2\"  }\n]");
    return table.rows.size() == 1 && table.rows.at(0) == std::vector<std::string>{"1", "2"};
}

auto test_parse_escapes() -> bool
{
    const auto table = json::parse(R"([{"s":"a\nb\tc\"d\\e\/f\b\r\f"}])");
    return table.rows.at(0).at(0) == std::string{"a\nb\tc\"d\\e/f\b\r\f"};
}

auto test_parse_unicode_bmp() -> bool
{
    const auto table = json::parse(R"([{"s":"\u00e9"}])");
    // U+00E9 "é" — two bytes in UTF-8.
    return table.rows.at(0).at(0) == "\xc3\xa9";
}

auto test_parse_unicode_surrogate_pair() -> bool
{
    // U+1F600 GRINNING FACE — four bytes in UTF-8.
    const auto table = json::parse(R"([{"s":"\uD83D\uDE00"}])");
    return table.rows.at(0).at(0) == "\xf0\x9f\x98\x80";
}

auto test_write_pretty() -> bool
{
    const core::Table table{.headers = {"a", "b"}, .rows = {{"1", "2"}}};
    const std::string expected = R"([
  {
    ...
    return json::write(table) == expected;
}

auto test_write_escapes() -> bool
{
    const core::Table table{.headers = {"s"}, .rows = {{"a\"b\\c\nd\te"}}};
    const std::string out = json::write(table);
    return out.contains(R"("s": "a\"b\\c\nd\te")");
}

auto test_write_control_char() -> bool
{
    const core::Table table{.headers = {"s"}, .rows = {{std::string{'\x01'}}}};
    const std::string out = json::write(table);
...
{
    const core::Table table{.headers = {"a"}, .rows = {}};
    return json::write(table) == "[]";
}

auto test_rejects_number_value() -> bool
{
    return expect::expect_throws([]() -> void { (void)json::parse(R"([{"a":3}])"); });
}

auto test_rejects_null_value() -> bool
{
    return expect::expect_throws([]() -> void { (void)json::parse(R"([{"a":null}])"); });
}

auto test_rejects_duplicate_keys() -> bool
{
    return expect::expect_throws([]() -> void { (void)json::parse(R"([{"a":"1","a":"2"}])"); });
}

auto test_rejects_empty_array() -> bool
{
    return expect::expect_throws([]() -> void { (void)json::parse("[]"); });
}

auto test_rejects_mismatched_keys() -> bool
{
    return expect::expect_throws([]() -> void { (void)json::parse(R"([{"a":"1","b":"2"},{"a":"3","c":"4"}])"); });
}

auto test_rejects_trailing_content() -> bool
{
    return expect::expect_throws([]() -> void { (void)json::parse(R"([{"a":"1"}]trailing)"); });
}
...
auto test_parse_unicode_3byte() -> bool
{
    ...
    const auto table = json::parse(R"([{"s":"\u2603"}])");
    return table.rows.at(0).at(0) == "\xe2\x98\x83";
}
...
auto test_write_backspace_formfeed_return() -> bool
{
    ...
    const core::Table table{.headers = {"s"}, .rows = {{std::string{'\b', '\f', '\r'}}}};
    const std::string out = json::write(table);
...
auto test_parse_empty_object() -> bool
{
    ...
    const auto table = json::parse("[{}]");
    return table.headers.empty() && table.rows.size() == 1 && table.rows.at(0).empty();
}
...
{
    return expect::expect_throws([]() -> void { (void)json::parse(R"([{"a":"\u00GZ"}])"); });
}
...
auto test_rejects_truncated_hex_escape() -> bool
{
    ...
    return expect::expect_throws([]() -> void { (void)json::parse(R"([{"a":"\u12)"); });
}

auto test_rejects_unpaired_high_surrogate() -> bool
{
    return expect::expect_throws([]() -> void { (void)json::parse(R"([{"a":"\uD800"}])"); });
}

auto test_rejects_high_with_bad_low_surrogate() -> bool
{
    return expect::expect_throws([]() -> void { (void)json::parse(R"([{"a":"\uD800\u0041"}])"); });
}
...
{
    return expect::expect_throws([]() -> void { (void)json::parse(R"([{"a":"\z"}])"); });
}
...
{
    return expect::expect_throws([]() -> void { (void)json::parse(R"([{"a":"hello)"); });
}

auto test_rejects_bad_separator_in_object() -> bool
{
    return expect::expect_throws([]() -> void { (void)json::parse(R"([{"a":"1" "b":"2"}])"); });
}

auto test_rejects_wrong_key_count() -> bool
{
    return expect::expect_throws([]() -> void { (void)json::parse(R"([{"a":"1","b":"2"},{"a":"3"}])"); });
}

auto test_rejects_bad_separator_in_array() -> bool
{
    return expect::expect_throws([]() -> void { (void)json::parse(R"([{"a":"1"} x])"); });
}
